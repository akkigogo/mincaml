...
tmp.8916:
	...
	lwc1	$f1, 96($sp)
	c.lt.s	$a0, $a0, $a1
	bne	$a0, $zero, bne_else.8917
	jr	$ra
bne_else.8917:
	lw	$a0, 108($sp)
	slti	$s0, $a0, 4
	beq	$s0, $zero, bne_else.8919
	addi	$a1, $a0, 1
	addi	$a2, $zero, -1
	sll	$a1, $a1, 2
	lw	$a3, 120($sp)
